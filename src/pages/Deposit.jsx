...
const methods = [
  { id: 'bank', name: 'Bank Transfer', icon: '🏦', desc: 'Free • 1-3 min processing' },
  { id: 'usdt', name: 'USDT (TRC20)', icon: '💎', desc: 'Instant • Min 100 USDT' },
    ...
    <div style={{ padding: '24px 32px 50px', maxWidth: 900, margin: '0 auto' }}>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 0' }}>
          ...
          <span style={{ color: 'var(--gold)', fontWeight: 600 }}>{methods.find(m => m.id === method)?.icon} {methods.find(m => m.id === method)?.name}</span>
        </div>
        ...
        <div style={{
          ...
          🎁 Welcome bonus: +<strong>${(custom ? parseInt(custom) : amount).toLocaleString()}</strong> free with first deposit!
        </div>
      ...
      {showSuccess && (
        <motion.div initial={{ y: 80, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 80, opacity: 0 }}
          ...
          ✅ ${(custom ? parseInt(custom) : amount).toLocaleString()} deposited successfully!
        </motion.div>
      )}
    </div>
  );
